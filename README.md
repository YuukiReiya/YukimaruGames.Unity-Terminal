...
> [!NOTE]
> **macOS**: `CLI(cmd,zsh)` が自動起動したTerminal.appのウィンドウは、セッション終了時に
> 自動で閉じます。ただしセッションを一度も使わずに(コマンド入力待ちのまま)終了した場合、
> Terminal.appの仕様上「実行中のプロセスを終了しますか?」という確認が必要になるため、
> 自動でそのボタンを押すには**Unity Editor(および本機能を使うPlayerビルド)へ
> アクセシビリティ権限を付与する必要があります**(システム設定 > プライバシーとセキュリティ >
> アクセシビリティ)。権限が無い場合、ウィンドウは自動で閉じず確認ダイアログが残るため、
> 手動で閉じてください。
