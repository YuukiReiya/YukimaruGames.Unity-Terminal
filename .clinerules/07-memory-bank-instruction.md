...
`memory-bank/` はGitリポジトリで管理し、他の利用者にも共有される。
そのため、以下の情報は `memory-bank/` 内のファイルに**記載してはならない**:

- ユーザー個人に依存する絶対パス（例: `C:\Users\<ユーザー名>\...`）
- ユーザー名・メールアドレス等の個人情報
- ローカル環境固有の設定値

このような情報が必要な場合は、`.gitignore` 対象の別ファイルで管理すること。
