...
namespace YukimaruGames.Terminal.Adapters.ExternalTerminal
{
    ...
    /// </remarks>
    public sealed class MacExternalTerminalLauncher : IExternalTerminalLauncher
    {
        public bool IsSupported => true;

        public Process Launch(int port)
        {
            var relayPath = RelayScriptWriter.WriteMacRelayScript();
            MakeExecutable(relayPath);

            var launcherPath = RelayScriptWriter.WriteMacLauncherScript();
