...
4.  **強制実施事項:**  
   - `.gitignore` に `.agents/` ディレクトリが記載されていない場合、**必ず** `.gitignore` に追加する。
   - `--author` フラグを**必ず**使用してコミット実行すること。`--author` フラグの使用を省略したコミットは禁止。
...
## 1.4 Gitコマンドの実行品質 (Anti-Freeze & Filter)
AgentがGitコマンドを実行する際は、プロセスの停滞を防ぎ、ユーザー指定の範囲を正確に反映するため以下のルールを厳守する。

- **ページャーの無効化:** 出力が長いコマンド（`status`, `diff`, `log`等）を実行する際は、ユーザーの入力を待機するページャーを回避するため、必ず `--no-pager` オプションを付与する。
  - 例: `git --no-pager diff`
...
## 5. 応答言語
- **重要:** ユーザーへの返答は、`03-cline-behavior.md` の規定に従うこと。

## 6. 追加ルール: Push実行前の承認

- **`git push` 実行前には必ずユーザーの明示的な承認を取得すること**  