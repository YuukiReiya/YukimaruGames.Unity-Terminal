...
**重要:** `git commit` 実行時に必ず `--author` フラグを使用し、以下を遵守する:

- `.agents/git-credentials.txt` から取得した `[NAME] <[EMAIL]>` を使用
- `--no-edit` フラグでメッセージを自動生成
- `.gitignore` に `.agents/` ディレクトリが記載されていることを前提とする